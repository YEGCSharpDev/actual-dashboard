        ...
        if fcntl:
            try:
                ...
                try:
                    ...
                    lock_file.close()
                    return {
                        "error": "Another process is currently syncing. Please wait.",
                        "accounts": [],
                        "transactions": pd.DataFrame(),
                        "budgets": {},
                    }

                res = _invoke_sidecar(args, env)
                ...
                st.warning(
                    f"Insufficient permissions to create lock file at {lock_path}. "
                    "Proceeding without lock."
                )
                return _invoke_sidecar(args, env)
            except Exception as e:
                return {
                    "error": f"Locking failed: {str(e)}",
                    "accounts": [],
                    "transactions": pd.DataFrame(),
                    "budgets": {},
                }
            ...
            st.warning(
                ...
                "Concurrent syncs may be unsafe."
            )
            return _invoke_sidecar(args, env)
...
def _invoke_sidecar(args, env) -> DashboardData:
    ...
    try:
        ...
        try:
            ...
            process.communicate()
            return {
                "error": "Sidecar timed out — check server connectivity",
                "accounts": [],
                "transactions": pd.DataFrame(),
                "budgets": {},
            }
...
def query_local_db(query: str, params: tuple = ()) -> list:
    ...
    if not _cached_db_path:
        raise FileNotFoundError(
            "Local Actual database (db.sqlite) not found. Run sync first."
        )
...
def get_investment_balances(all_data: dict) -> dict:
    ...
    # P1-1: Use identifier from secrets
    tfsa_id = st.secrets["tfsa"]["base"]["identifier"].upper()
...
def fetch_underbudgeted_amounts(all_data: dict) -> tuple[dict, list, str | None]:
    """
    Calculate underfunded amounts by querying the 'zero_budgets' view in the
    local SQLite DB.
    """
    now = datetime.now()
