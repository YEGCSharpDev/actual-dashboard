...
def build_net_worth_series(
    df_all: pd.DataFrame, current_balance: float
) -> pd.DataFrame:
    """
    ...
    # Guard: If data exists but current balance is negligible, we likely have
    # all accounts excluded. Return empty to avoid a misleading negative wealth trend.
    # (P2-P, P2-S)
    ...
    monthly = (
        df_sorted.set_index("date").resample("ME")["amount_dollars"].sum().reset_index()
    )
...
def calculate_mom_metrics(
    df_current: pd.DataFrame, df_prev: pd.DataFrame, current_date: datetime
) -> dict:
    """
...
def calculate_yoy_metrics(
    df_current: pd.DataFrame, df_last_year: pd.DataFrame, current_date: datetime
) -> dict:
    """
...
def calculate_budget_pacing(
    spent: float, budget: float, current_date: datetime
) -> float:
    """
...
def calculate_milestone_months(
    ...
    if monthly_rate == 0:
        ...
        return int(needed / monthly_savings) + (
            1 if needed % monthly_savings > 0 else 0
        )
...
def build_progress_bar_html(
    ...
    return (
        ...
        f'<div style="width: 85px; font-weight: bold; '
        f'color: {color_solid}; font-size: 14px;">'
        f"{safe_label}</div>"
        ...
        f'<span style="color: #999999; font-size: 12px; font-weight: bold; '
...
def build_category_bar_html(
    ...
    return (
        ...
        f'<div style="position: relative; background-color: {bg_color}; '
        f"border-radius: 6px; "
        f"height: 26px; width: 100%; border: 1px solid {color}40; "
        f'overflow: hidden;">'
        f"{marker_html}"
